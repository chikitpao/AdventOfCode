...
getRobotPos :: (Int, Int, Char) -> (Int, Int)
getRobotPos (a, b, _) = (a, b)
getRobotDir :: (Int, Int, Char) -> Char
getRobotDir (_, _, a) = a
...
-- REMARK: `sequence' and `sequence_' are defined in `Data.Foldable' and imported from `Prelude'.
-- So don't use them (variable renamed to alignmentParameters).
evaluateScaffolds :: [[Char]] -> (Int, Int) -> [Char] -> (Turn, Direction) -> [Char] -> [Char]
evaluateScaffolds input prevPos alignmentParameters prevTurn scaffolds =
    if null scaffolds then
        alignmentParameters
    else
        let sep = if null alignmentParameters then "" else "," 
            turn = fst prevTurn
            turnChar = if turn == LeftTurn then "L" else "R"
            direction = snd prevTurn
            nextSequence = alignmentParameters ++ sep ++ turnChar ++ "," ++ show (length scaffolds)
...
getMainSequence input prevPos alignmentParameters prevTurn = 
    let columnCount = length (head input)
        rowCount = length input
        direction = snd prevTurn
        prevX = fst prevPos
        ...
        if direction == North && prevY > 0 then
            evaluateScaffolds input prevPos alignmentParameters prevTurn $ takeWhile (== '#') [input !! y !! prevX | y <- [prevY-1, prevY-2..0], y >= 0]
        else if direction == East && prevX + 1 < columnCount then
           evaluateScaffolds input prevPos alignmentParameters prevTurn $ takeWhile (== '#') [input !! prevY !! x | x <- [prevX+1..columnCount-1], x < columnCount]
        else if direction == South && prevY + 1 < rowCount then
            evaluateScaffolds input prevPos alignmentParameters prevTurn $ takeWhile (== '#') [input !! y !! prevX | y <- [prevY+1..rowCount-1], y < rowCount]
        else if direction == West && prevX > 0 then
            evaluateScaffolds input prevPos alignmentParameters prevTurn $ takeWhile (== '#') [input !! prevY !! x | x <- [prevX-1, prevX-2..0], x >= 0]
        else
...
main = do
    ...
    let programState = createProgramState2 program 0 Nothing Nothing 0
        ...
        output1 = convertToMap $ fromJust $ stateOutput (fromJust programResult)
{-
    mapM_ print output1
    putStrLn ""
-}
    ...
    print $ "Answer 1: " ++ show (countIntersections output1)
    ...
    let completeSequence = getMainSequence output1 (getRobotPos robot) "" (getFirstTurn output1 robot)
    ...
    let mainRoutine = map ord "A,B,A,C,B,C,B,A,C,B\n"
        ...
        output2 = fromJust $ stateOutput $ fromJust programState3
    print $ "Answer 2: " ++ show (last output2)
